...
* Quando usamos regressão linear estamos em busca da reta que melhor se ajusta aos pontos que vimos no gráfico de dispersão, lembrando que a **equação de uma reta** é da forma _y=a + bx_ temos que os coeficientes da nossa reta (em _Coefficients_, na coluna _Estimate_) são:
   + *a*= 53.17654 (**Coenficiente linear** | onde a reta **intercepta** o eixo y, quando x=0)
   + *b*= 1.06542 (**Coeficiente angular** | Inclinação da reta)

* A coluna _Std. Error_ é o desvio padrão residual relacionado a cada coeficiente calculado (estimado), esse parãmetro serve para calcular os intervalos de confiança para cada coeficiente;

* A coluna _t value_ é a medida de a quantos desvios padrão a estimativa de coefeciente está longe de zero, esse dado pode ser usado para calcular o _p-valor_;
